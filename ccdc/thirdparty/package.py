...
class Package(object):
    ...
    def centos_major_version(self):
        return (
            subprocess.check_output("rpm -E %{rhel}", shell=True)
            .decode("utf-8")
            .strip()
        )
    ...
    def ubuntu(self):
        return (
            self.debian
            and subprocess.check_output("lsb_release -i -s", shell=True)
            .decode("utf-8")
            .strip()
            == "Ubuntu"
        )
    ...
    def ubuntu_version(self):
        return (
            subprocess.check_output("lsb_release -r -s", shell=True)
            .decode("utf-8")
            .strip()
        )
    ...
    def extract_source_archives(self):
        for source_archive_filename in self.source_archives.keys():
            self.extract_archive(
                self.source_downloads_base / source_archive_filename,
                self.source_extracted,
            )
    ...
    def run_configuration_script(self):
        ...
        self.system(
            [str(self.configuration_script)] + self.arguments_to_configuration_script,
    ...
    def change_dylib_lookup(self, library_path, from_path, to_path):
        ...
        self.system(
            ["install_name_tool", "-change", from_path, to_path, str(library_path)]
        )
...
class CMakeMixin(Package):
    ...
    def run_build_command(self):
        self.system(
            [self.configuration_script, "--build", ".", "--config", "Release"],
            env=self.environment_for_build_command,
            cwd=self.build_directory_path,
        )
    ...
    def run_install_command(self):
        self.system(
            [self.configuration_script, "--install", "."],
            env=self.environment_for_build_command,
            cwd=self.build_directory_path,
        )
    ...
    def visual_studio_generator_for_build(self):
        if "BUILD_VS_VERSION" not in os.environ:
        ...
        raise Exception(
            f'Invalid value for BUILD_VS_VERSION: {os.environ["BUILD_VS_VERSION"]}'
        )
