...
from ccdc.thirdparty.package import (
    Package,
    AutoconfMixin,
    MakeInstallMixin,
    NoArchiveMixin,
...
class ZlibPackage(
    InstallInConquestPythonBaseMixin, AutoconfMixin, NoArchiveMixin, Package
):
    """Zlib"""
...
class OpensslPackage(
    ...
    def run_install_command(self):
        ...
        self.system(
            ["make", "install_sw"],
            env=self.environment_for_build_command,
            cwd=self.build_directory_path,
        )
        ...
        shutil.copyfile(
            self.install_directory / "lib" / "libssl.a", static_libs_path / "libssl.a"
        )
        shutil.copyfile(
            self.install_directory / "lib" / "libcrypto.a",
            static_libs_path / "libcrypto.a",
        )
...
class DbPackage(
    InstallInConquestPythonBaseMixin, MakeInstallMixin, NoArchiveMixin, Package
):
    ...
    @property
    def arguments_to_configuration_script(self):
        args = ["--enable-compat185", "--enable-shared", "--enable-dbm"]
        if self.macos:
...
class TiffPackage(
    InstallInConquestPythonBaseMixin, CMakeMixin, NoArchiveMixin, Package
):
    name = "tiff"
    version = "4.2.0"
...
class ConquestPythonPackage(AutoconfMixin, MakeInstallMixin, Package):
    ...
    def patch_sources(self):
        self.patch(
            ...
            ("/usr/contrib/ssl/lib/", f"{OpensslPackage().install_directory}/lib"),
        )
    ...
    def ldflags(self):
        ...
        if self.macos:
            ...
            wanted_rpath = ":".join(
                str(x)
                for x in SqlitePackage().library_link_directories
    ...
    def pip_install(self, *args, env=dict(os.environ)):
        self.system(
            [str(self.python_exe), "-m", "pip", "install"] + [arg for arg in args],
            env=env,
        )
    ...
    def run_install_command(self):
        ...
        self.system(
            [
                ...
                "/a",
                f"{installer}",
                f"TARGETDIR={self.install_directory}",
    ...
    def smoke_test(self):
        subprocess.check_call([self.python_exe, "smoke_test.py"])
...
def main():
    ...
    if Package().linux:
        ...
        subprocess.run("sudo dnf install -y libXmu-devel", shell=True, check=True)
    ...
    if Package().linux:
        command = [
            ...
            "-i",
            os.path.join(
                os.path.dirname(os.path.abspath(__file__)), "pyinstaller.patch"
            ),
        ]
    ...
    if not Package().windows:
        ...
        ConquestPythonPackage().pip_install(
            ...
            f"--global-option={ApswPackage().version}",
            "--global-option=--all",
            "--global-option=build",
        ...
        # install apsw
        site_packages_dir = (
            ConquestPythonPackage().install_directory / "Lib" / "site-packages"
        )
        apsw_installer = (
            ApswPackage().source_downloads_base
            / "conquest-windows-build-requirements"
        ...
        togl_zip = (
            ToglPackage().source_downloads_base
            / "conquest-windows-build-requirements"
            / "Togl-2.2-pre.zip"
        )
        ConquestPythonPackage().system(
        ...
        precompiled_bsddb3 = (
            DbPackage().source_downloads_base
            / "conquest-windows-build-requirements"
